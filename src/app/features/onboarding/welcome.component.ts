...
import { form, email, minLength, required, FormField } from '@angular/forms/signals';
...
@Component({
  ...
  template: `
    <div class="min-h-screen flex flex-col items-center justify-center px-6 bg-cream relative">
      <div class="w-full max-w-sm">
        ...
        @if (!auth.isLoggedIn()) {
          ...
          @switch (authMode()) {
            ...
            @case ('login') {
              <div class="flex flex-col gap-4">
                <button
                  (click)="authMode.set('initial')"
                  class="text-green-primary font-medium text-sm active:opacity-70 self-start min-h-11"
                >
                  ‹ Nazad
                </button>
          ...
          <div class="flex flex-col items-center gap-3 py-8">
            <div
              class="w-8 h-8 border-3 border-green-primary border-t-transparent rounded-full animate-spin"
            ></div>
          ...
          @switch (mode()) {
            ...
            @case ('create') {
              <div class="flex flex-col gap-4">
                <button
                  (click)="mode.set('choice')"
                  class="text-green-primary font-medium text-sm active:opacity-70 self-start min-h-[44px]"
                >
                  ‹ Nazad
                </button>
      ...
      <p
        class="absolute bottom-0 left-0 right-0 text-center text-xs text-text-muted font-mono pb-[max(1rem,env(safe-area-inset-bottom))]"
      >
...
export class WelcomeComponent {
  ...
  readonly credentialsModel = signal({ name: '', email: '', password: '' });
  readonly credentials = form(this.credentialsModel, (path) => {
    required(path.email);
    email(path.email);
  ...
  readonly joinModel = signal({ code: '' });
  readonly joinForm = form(this.joinModel, (path) => {
    required(path.code);
    minLength(path.code, 6);
  });

  readonly canSignIn = computed(() => {
    const { email: mail, password } = this.credentialsModel();
    return mail.trim().length > 0 && password.length >= 6;
  });

  readonly canRegister = computed(
    () => this.canSignIn() && this.credentialsModel().name.trim().length > 0,
  );

  readonly canJoin = computed(() => this.joinModel().code.trim().length >= 6);
  ...
  async signInWithEmail(): Promise<void> {
    ...
    this.error.set('');
    try {
      await this.auth.signInWithEmail(this.credentialsModel().email.trim(), this.credentialsModel().password);
    } catch (err: any) {
      const code = err.code ?? '';
      let msg: string;
  ...
  async registerWithEmail(): Promise<void> {
    ...
    try {
      await this.auth.registerWithEmail(
        this.credentialsModel().email.trim(),
        this.credentialsModel().password,
        this.credentialsModel().name.trim(),
      );
    } catch (err: any) {
      const code = err.code ?? '';
      let msg: string;
  ...
  join(): void {
    const code = this.joinModel().code.trim().toUpperCase();
